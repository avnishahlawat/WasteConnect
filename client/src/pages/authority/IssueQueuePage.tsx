...
export default function IssueQueuePage() {
  ...
  useEffect(() => {
    fetchIssues();
  }, [status, severity, isOverdue, serviceArea]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Municipal Issue Queue"
        subtitle="Manage, triage, and dispatch crews for citizen-reported sanitation reports."
        breadcrumbs={[
        ...
        <form onSubmit={handleSearchSubmit} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by title, description, address, or category..."
              className="w-full h-10 pl-9 pr-3 bg-background border border-border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
            />
          </div>
          <button
            type="submit"
            className="px-4 py-2 bg-primary text-white text-sm font-medium rounded-md hover:bg-primary-dark"
          >
            Search
          </button>
        </form>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1 text-xs">
          <div>
            ...
            <select
              ...
              className="w-full h-9 px-2 bg-background border border-border rounded-md focus:outline-none"
            >
      ...
      {loading ? (
        ...
        <div className="bg-surface border border-border rounded-xl p-8">
          <EmptyState
            icon={List}
            title="No Issues Match Query"
            description="Adjust your filters or search keywords to view other municipal reports."
          />
        </div>
      ) : (
              ...
              {issues.map((i) => (
                <tr key={i._id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-4">
                    ...
                    {i.isOverdue && (
                      <div className="mt-1 text-[10px] text-red-600 font-bold">OVERDUE SLA</div>
                    )}
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    ...
                    <div className="text-text-muted text-[11px]">{(i.serviceArea as any)?.name || 'Central'}</div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    <Link
                      to={`/authority/issues/${i._id}`}
                      className="inline-flex items-center gap-1 px-3 py-1.5 bg-primary text-white font-medium text-xs rounded hover:bg-primary-dark shadow-sm"
                    >
