...
export default function CityMapPage() {
    ...
    const filteredIssues = issues.filter((i) => {
        if (filter === 'ACTIVE')
            return !['RESOLVED', 'CLOSED', 'REJECTED'].includes(i.status);
        if (filter === 'OVERDUE')
            return i.isOverdue;
        if (filter === 'RESOLVED')
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader title="Greenfield Municipal City Map" subtitle="Live geospatial distribution of public waste reports and service area hotspot perimeters." breadcrumbs={[
        ...
        <div className="flex items-center bg-surface border border-border p-1 rounded-lg text-xs font-semibold">
          <button onClick={() => setFilter('ACTIVE')} className={`px-3 py-1.5 rounded-md transition-colors ${filter === 'ACTIVE' ? 'bg-primary text-white shadow-sm' : 'text-text-muted hover:text-text'}`}>
            Active Incidents ({issues.filter((i) => !['RESOLVED', 'CLOSED'].includes(i.status)).length})
          </button>
          <button onClick={() => setFilter('OVERDUE')} className={`px-3 py-1.5 rounded-md transition-colors ${filter === 'OVERDUE' ? 'bg-status-error text-white shadow-sm' : 'text-text-muted hover:text-text'}`}>
            Overdue ({issues.filter((i) => i.isOverdue).length})
          </button>
          <button onClick={() => setFilter('RESOLVED')} className={`px-3 py-1.5 rounded-md transition-colors ${filter === 'RESOLVED' ? 'bg-status-success text-white shadow-sm' : 'text-text-muted hover:text-text'}`}>
            Resolved
          </button>
          <button onClick={() => setFilter('ALL')} className={`px-3 py-1.5 rounded-md transition-colors ${filter === 'ALL' ? 'bg-charcoal text-white shadow-sm' : 'text-text-muted hover:text-text'}`}>
            All Pins ({issues.length})
          </button>
        </div>
      </div>

      {loading ? (<LoadingSkeleton type="card"/>) : (<div className="space-y-4">
          <IssuesMap issues={filteredIssues} serviceAreas={serviceAreas} height="620px" detailBaseUrl="/authority/issues"/>
