...
export default function AuthorityAnalyticsPage() {
  ...
  const wasteTrends =
    data?.wasteStats?.trends?.map((t: any) => ({
  ...
  const categoryBreakdown =
    data?.wasteStats?.categoryBreakdown?.map((c: any) => ({
      name: c._id,
  ...
  const issueTrends =
    data?.issueStats?.issuesByMonth?.map((i: any) => ({
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <PageHeader
        title="Municipal Operations Intelligence"
        subtitle="Historical reporting trends, waste diversion volume, and service area resolution metrics."
        breadcrumbs={[
