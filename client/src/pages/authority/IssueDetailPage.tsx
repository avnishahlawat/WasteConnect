...
import type { PublicIssue, IssueEvent } from '../../types';
import {
  ArrowLeft,
  MapPin,
  Edit,
...
export default function AuthorityIssueDetailPage() {
  ...
  const [data, setData] = useState<{ issue: PublicIssue; events: IssueEvent[] } | null>(null);
  ...
  if (!data?.issue) return <div className="p-8 text-center text-text-muted">Issue not found.</div>;

  const { issue, events } = data;
  const coords: [number, number] = [issue.location.coordinates[1], issue.location.coordinates[0]];
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-surface p-6 rounded-xl border border-border shadow-sm space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {issue.isOverdue && (
            <span className="text-xs bg-red-100 text-red-700 font-bold px-2 py-0.5 rounded border border-red-200">
              SLA BREACHED
            </span>
          )}
        ...
        <div className="text-xs text-text-muted pt-2 border-t border-border">
          Reported: {formatDateTime(issue.createdAt)} • Ward: {(issue.serviceArea as any)?.name || 'Central'} • Reporter:{' '}
          <strong className="text-charcoal">
            {(issue.reporter as any)?.firstName} {(issue.reporter as any)?.lastName} ({(issue.reporter as any)?.email})
          </strong>
        </div>
      </div>
          ...
          <form onSubmit={handleTriageSubmit} className="space-y-3 text-xs">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Sanitation Unit 4, Heavy Equipment Crew"
                className="w-full h-9 px-3 bg-background border border-border rounded-md text-xs focus:outline-none"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Private operational notes not visible to citizens..."
                className="w-full p-2 bg-background border border-border rounded-md text-xs focus:outline-none"
              />
            </div>
          ...
          <form onSubmit={handleResolveSubmit} className="space-y-3 text-xs">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Detail what equipment was deployed, total debris removed, and confirmation that the area has been cleared..."
                className="w-full p-2 bg-background border border-border rounded-md text-xs focus:outline-none"
                required
              />
            </div>
            ...
            <button
              type="submit"
              disabled={submittingAction || !resolutionNotes}
              className="w-full py-2 bg-status-success text-white font-semibold rounded-md hover:bg-green-700 transition-colors disabled:opacity-50"
            >
            ...
            <div key={ev._id} className="py-3 flex items-start justify-between gap-4 text-xs">
              <div>
                ...
                <div className="text-gray-400 mt-1">
                  Actor: {(ev.actor as any)?.firstName || 'Municipal Staff'} ({ev.actorRole})
                </div>
              </div>
