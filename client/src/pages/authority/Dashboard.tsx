...
export default function AuthorityDashboard() {
  const [data, setData] = useState<{
    ...
    urgentIssues: PublicIssue[];
    hotspotSummary: ServiceArea[];
  } | null>(null);
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader
          ...
          subtitle="Triage public reports, coordinate field teams, and manage hotspot remediation."
          className="border-0 pb-0 mb-0"
        />
        <div className="flex items-center gap-3">
          <Link
            to="/authority/map"
            className="flex items-center gap-1.5 bg-surface border border-border text-charcoal text-xs font-semibold px-4 py-2 rounded-md hover:bg-gray-50 transition-colors shadow-sm"
          >
      ...
      {metrics.overdueCount > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <Link
            to="/authority/issues?isOverdue=true"
            className="text-xs font-bold text-red-800 underline whitespace-nowrap self-center"
          >
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <MetricCard
          ...
          subtitle={`Score: ${metrics.slaComplianceRate}% SLA`}
          icon={Flame}
          variant="error"
        />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-surface rounded-xl border border-border p-6 shadow-sm">
          ...
          {!data?.urgentIssues?.length ? (
            <EmptyState
              icon={CheckCircle2}
              title="Queue Clear"
              description="No unresolved issues currently pending in your jurisdiction."
            />
          ) : (
            <div className="divide-y divide-border">
              {data.urgentIssues.map((issue) => (
                <div key={issue._id} className="py-3.5 flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-text-muted">
                      {issue.address} • {(issue.serviceArea as any)?.name || 'Central'} • {formatDate(issue.createdAt)}
                    </div>
                  </div>
                  <Link
                    to={`/authority/issues/${issue._id}`}
        ...
        <div className="bg-surface rounded-xl border border-border p-6 shadow-sm">
          ...
          <div className="space-y-4">
            {data?.hotspotSummary?.map((area) => (
              <div key={area._id} className="p-3 bg-background border border-border rounded-lg space-y-2">
                ...
                <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(100, Math.max(5, area.hotspotScore))}%` }}
                  />
                </div>
              </div>
            ))}
