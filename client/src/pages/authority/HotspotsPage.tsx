...
import { authorityApi, type HotspotSummary } from '../../api/authority';
import { formatDate } from '../../lib/utils';
import { RefreshCw, Calculator, Info } from 'lucide-react';

export default function HotspotsPage() {
  const [data, setData] = useState<HotspotSummary | null>(null);
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader
          title="Area Hotspot Risk Index"
          subtitle="Dynamic municipal solid waste accumulation and recurrence index."
          breadcrumbs={[
        ...
        <button
          ...
          className="inline-flex items-center gap-2 bg-primary text-white text-xs font-semibold px-4 py-2.5 rounded-md hover:bg-primary-dark transition-colors disabled:opacity-50 shadow-sm"
        >
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {data?.areas?.map((area) => (
            <div
              key={area._id}
              className="bg-surface border border-border rounded-xl p-6 shadow-sm hover:border-gray-300 transition-colors space-y-4"
            >
              ...
              <div>
                ...
                <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(100, Math.max(8, area.hotspotScore))}%` }}
                  />
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-text-muted">
                ...
                {area.hotspotLastCalculated && (
                  <span>Updated {formatDate(area.hotspotLastCalculated)}</span>
                )}
              </div>
            </div>
          ))}
