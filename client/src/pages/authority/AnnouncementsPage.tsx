...
export default function AuthorityAnnouncementsPage() {
  ...
  const [type, setType] = useState<'INFO' | 'WARNING' | 'ALERT' | 'SCHEDULE'>('INFO');
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader
          title="Civic Announcements"
          subtitle="Broadcast notices, schedule changes, and weather advisories to citizens and collectors."
          breadcrumbs={[
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 bg-primary text-white text-xs font-semibold px-4 py-2.5 rounded-md hover:bg-primary-dark transition-colors shadow-sm"
        >
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {announcements.map((a) => {
            ...
            return (
              <div key={a._id} className="bg-surface border border-border rounded-xl p-5 shadow-sm space-y-2">
                ...
                <div className="text-xs text-text-muted pt-2 border-t border-border flex items-center gap-4">
                  ...
                  {a.targetServiceArea && (
                    <span>Zone: <strong className="text-charcoal">{a.targetServiceArea.name}</strong></span>
                  )}
            ...
            <form onSubmit={handleCreate} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Schedule Update: East Ward Collection Timing"
                  className="w-full h-10 px-3 bg-background border border-border rounded-md text-sm focus:outline-none"
                  required
                />
              </div>
              ...
              <div>
                ...
                <select
                  value={targetServiceArea}
                  onChange={(e) => setTargetServiceArea(e.target.value)}
                  className="w-full h-9 px-2 bg-background border border-border rounded-md text-xs focus:outline-none"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Provide complete details, affected streets, instructions, or contact channels..."
                  className="w-full p-2.5 bg-background border border-border rounded-md text-xs focus:outline-none"
                  required
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 bg-primary text-white text-xs font-semibold rounded hover:bg-primary-dark disabled:opacity-50"
                >
