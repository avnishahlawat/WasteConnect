...
export default function CollectorDashboard() {
  ...
  const [data, setData] = useState<{
    ...
    recentAssignments: PickupRequest[];
    recentFeedback: any[];
  } | null>(null);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader
          title={`Collector Hub • ${user?.firstName || 'Collector'}`}
          subtitle="View active assignments and browse open pickups in your authorized service zones."
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${
                profile.availability === 'AVAILABLE' ? 'bg-status-success animate-pulse' : 'bg-gray-400'
              }`}
            />
          ...
          <Link
            ...
            className="flex items-center gap-1.5 bg-primary text-white text-xs font-semibold px-4 py-2 rounded-md hover:bg-primary-dark transition-colors shadow-sm"
          >
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard
          title="Active Route Pickups"
          value={metrics.assignedPickups}
          subtitle={`${metrics.inProgressPickups} currently in transit`}
          icon={Truck}
          variant="info"
        />
        <MetricCard
          ...
          subtitle="Collections fulfilled"
          icon={CheckCircle2}
          variant="success"
        />
        <MetricCard
          title="Marketplace Requests"
          value={metrics.availableInArea}
          subtitle="Available in your service area"
          icon={Inbox}
          variant="warning"
        />
        <MetricCard
          ...
          subtitle={`From ${profile.totalRatings} citizen reviews`}
          icon={Star}
          variant="default"
        />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-surface rounded-xl border border-border p-6 shadow-sm">
          ...
          {!data?.recentAssignments?.length ? (
            <EmptyState
              icon={Truck}
              title="No Pickups on Route"
              description="Browse the marketplace to accept new waste collection jobs."
              action={
                <Link
                  to="/collector/available"
                  className="inline-flex items-center gap-2 bg-primary text-white text-xs font-medium px-3 py-1.5 rounded-md mt-2"
                >
            ...
            <div className="divide-y divide-border">
              {data.recentAssignments.map((p) => (
                <div key={p._id} className="py-3.5 flex items-center justify-between">
                  <div>
                    <div className="font-medium text-sm text-text">
                      {(p.wasteCategory as any)?.name || 'Waste'} ({p.estimatedQuantity} {p.unit})
                    </div>
                    <div className="text-xs text-text-muted mt-0.5">
                      {p.address} • Date: {formatDate(p.preferredDate)} ({p.timeSlot})
                    </div>
                    <div className="text-[11px] text-charcoal mt-1">
                      Citizen: {(p.citizen as any)?.firstName} {(p.citizen as any)?.lastName} ({(p.citizen as any)?.phone || 'No phone'})
                    </div>
                  </div>
        ...
        <div className="bg-surface rounded-xl border border-border p-6 shadow-sm">
          ...
          {!data?.recentFeedback?.length ? (
            <EmptyState
              ...
              description="Complete pickups and citizens will provide service feedback here."
            />
          ) : (
            <div className="divide-y divide-border">
              {data.recentFeedback.map((f: any) => (
                <div key={f._id} className="py-3.5 space-y-1">
                      ...
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          ...
                          className={`w-3.5 h-3.5 ${
                            s <= f.rating ? 'text-amber-400 fill-amber-400' : 'text-gray-200'
                          }`}
                        />
                      ))}
