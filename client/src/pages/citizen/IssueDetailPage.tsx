...
export default function IssueDetailPage() {
  ...
  if (!data?.issue) return <div className="p-8 text-center text-text-muted">Issue report not found.</div>;
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-surface p-6 rounded-xl border border-border shadow-sm space-y-3">
        ...
        <div className="text-xs text-text-muted pt-2 border-t border-border">
          Reported on {formatDateTime(issue.createdAt)} • Ward: {(issue.serviceArea as any)?.name || 'Central'}
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        ...
        <div className="bg-surface p-6 rounded-xl border border-border shadow-sm space-y-3">
          ...
          {issue.resolutionNotes ? (
            ...
            <div className="text-xs text-text-muted py-6">
            ...
            <div key={ev._id} className="py-3 flex items-start justify-between gap-4 text-xs">
              <div>
                ...
                <div className="text-gray-400 mt-1">
                  Updated by: {(ev.actor as any)?.firstName || 'Municipal Staff'} ({ev.actorRole})
                </div>
              </div>
