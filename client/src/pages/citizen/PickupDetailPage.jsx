...
export default function PickupDetailPage() {
    ...
    if (!data?.pickup)
        return <div className="p-8 text-center text-text-muted">Pickup request not found.</div>;
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      <Link to="/citizen/pickups" className="inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:underline">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-surface p-6 rounded-xl border border-border shadow-sm">
        <div>
          ...
          <h2 className="text-xl font-bold text-text">
            {pickup.wasteCategory?.name || 'Waste'} Collection
          </h2>
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-primary">
            {formatWeight(pickup.actualQuantity || pickup.estimatedQuantity, pickup.unit)}
          </div>
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="bg-surface p-6 rounded-xl border border-border shadow-sm space-y-3">
          ...
          <div className="text-xs space-y-1.5 text-text-muted">
            ...
            <div>
              <span className="font-medium text-text">Service Area:</span> {pickup.serviceArea?.name || 'Central'}
            </div>
            <div>
            ...
            {pickup.notes && (<div className="pt-2 border-t border-border">
        ...
        <div className="bg-surface p-6 rounded-xl border border-border shadow-sm space-y-3">
          ...
          {pickup.assignedCollector ? (<div className="text-xs space-y-1.5 text-text-muted">
              <div className="font-bold text-sm text-text">
                {pickup.assignedCollector.firstName} {pickup.assignedCollector.lastName}
              </div>
              <div>
                <span className="font-medium text-text">Phone:</span> {pickup.assignedCollector.phone || 'N/A'}
              </div>
              <div>
                <span className="font-medium text-text">Email:</span> {pickup.assignedCollector.email}
              </div>
              {feedback && (<div className="mt-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
            ...
            </div>) : (<div className="text-xs text-text-muted py-4">
          ...
          {events.map((ev) => (<div key={ev._id} className="py-3 flex items-start justify-between gap-4 text-xs">
              <div>
                ...
                <div className="text-gray-400 mt-1">
                  Actor: {ev.actor?.firstName || 'System'} ({ev.actorRole})
                </div>
              </div>
