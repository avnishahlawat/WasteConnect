...
import type { PickupRequest, PublicIssue } from '../../types';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader
          title={`Welcome back, ${user?.firstName || 'Citizen'}`}
          subtitle="Manage your household collections and report civic waste issues."
          className="border-0 pb-0 mb-0"
        />
        <div className="flex flex-wrap gap-3">
          <Link
            to="/citizen/request-pickup"
            className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-md font-medium text-sm hover:bg-primary-dark transition-colors shadow-sm"
          >
            <Plus className="w-4 h-4" /> Request Pickup
          </Link>
          <Link
            to="/citizen/report-issue"
            className="flex items-center gap-2 bg-surface border border-border text-charcoal px-4 py-2 rounded-md font-medium text-sm hover:bg-gray-50 transition-colors shadow-sm"
          >
      ...
      {data?.announcements && data.announcements.length > 0 && (
        <div className="bg-primary-muted/20 border border-primary/20 rounded-xl p-4 flex items-start gap-3">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard
          ...
          subtitle={`${metrics.activePickups} in progress`}
          icon={Truck}
          variant="default"
        />
        <MetricCard
          ...
          subtitle="Household waste diverted"
          icon={CheckCircle2}
          variant="success"
        />
        <MetricCard
          title="Public Issues Reported"
        ...
        <MetricCard
          ...
          subtitle="Municipal response rate"
          icon={CheckCircle2}
          variant="info"
        />
      </div>
        ...
        <div className="bg-surface rounded-xl border border-border p-6 shadow-sm">
          ...
          {!data?.recentPickups?.length ? (
            <EmptyState
              ...
              description="Schedule your first recyclable or bulk waste collection."
              action={
                <Link
                  to="/citizen/request-pickup"
        ...
        <div className="bg-surface rounded-xl border border-border p-6 shadow-sm">
          ...
          {!data?.recentIssues?.length ? (
            <EmptyState
              ...
              description="Help keep your neighborhood clean by reporting public waste accumulation."
              action={
                <Link
                  to="/citizen/report-issue"
                  className="inline-flex items-center gap-2 bg-primary text-white text-xs font-medium px-3 py-1.5 rounded-md mt-2"
            ...
            <div className="divide-y divide-border">
              {data.recentIssues.map((i) => (
                <div key={i._id} className="py-3.5 flex items-center justify-between">
                  <div>
