...
export default function AuditLogsPage() {
    ...
    return (<div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader title="Security & System Audit Trail" subtitle="Immutable operational record of platform authentication, triage events, and privileged actions." breadcrumbs={[
            { label: 'Admin Command', href: '/admin/dashboard' },
            { label: 'Audit Logs' },
        ]} className="border-0 pb-0 mb-0"/>

        <div className="w-full sm:w-64">
          <select value={action} onChange={(e) => setAction(e.target.value)} className="w-full h-9 px-3 bg-surface border border-border rounded-md text-xs focus:outline-none">
      ...
      {loading ? (<div className="space-y-3">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border text-xs">
              {logs.map((log) => (<tr key={log._id} className="hover:bg-gray-50/50">
