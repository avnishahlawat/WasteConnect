...
export default function AdminIssuesPage() {
    ...
    return (<div className="space-y-6 max-w-6xl mx-auto">
      <PageHeader title="Platform Public Issues" subtitle="Global audit list of all public sanitation and illegal dumping complaints." breadcrumbs={[
      ...
      {loading ? (<LoadingSkeleton type="card"/>) : (<div className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border text-xs">
              {issues.map((i) => (<tr key={i._id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-4 text-text-muted">{i.serviceArea?.name || 'Central'}</td>
                  <td className="px-5 py-4">
