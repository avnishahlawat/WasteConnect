...
export function ConfirmDialog({ open, onClose, onConfirm, title, description, confirmText = 'Confirm', variant = 'default' }) {
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-surface rounded-xl p-6 w-full max-w-md shadow-lg border border-border z-50">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              {(isDanger || isWarning) && (<div className={cn("p-2 rounded-full", isDanger ? "bg-status-errorBg text-status-error" : "bg-status-warningBg text-status-warning")}>
          ...
          <div className="flex justify-end gap-3">
            ...
            onConfirm();
            onClose();
        }} className={cn("px-4 py-2 text-sm font-medium rounded-md transition-colors text-white", isDanger ? "bg-status-error hover:bg-red-700" :
