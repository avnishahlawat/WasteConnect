...
export function MetricCard({ title, value, subtitle, icon: Icon, trend, variant = 'default', className }: MetricCardProps) {
  ...
  return (
    <div className={cn('bg-surface p-6 rounded-xl border border-border shadow-sm border-l-4', getBorderColor(), className)}>
      ...
      <div className="flex items-baseline gap-2">
        ...
        {trend !== undefined && (
          <div className={cn('text-sm font-medium flex items-center', trend >= 0 ? 'text-status-success' : 'text-status-error')}>
            {trend >= 0 ? '+' : ''}{trend}%
          </div>
        )}
      </div>
