...
export function IssuesMap({ issues, serviceAreas = [], height = '500px', detailBaseUrl = '/authority/issues', }) {
    // Center map on Greenfield
    const defaultCenter = [28.6139, 77.2090];
    return (<div style={{ height }} className="w-full rounded-xl overflow-hidden border border-border shadow-sm">
      <MapContainer center={defaultCenter} zoom={12} scrollWheelZoom={true} className="h-full w-full">
        ...
        {serviceAreas.map((area) => (<Circle key={area._id} center={[area.coordinates.lat, area.coordinates.lng]} radius={2500} pathOptions={{
        ...
        {issues.map((issue) => {
            ...
            return (<Marker key={issue._id} position={coords} icon={createCustomIcon(color)}>
              <Popup>
                <div className="p-1 max-w-xs space-y-2">
                  ...
                  <div className="pt-1">
                    <Link to={`${detailBaseUrl}/${issue._id}`} className="text-xs font-semibold text-primary hover:underline">
