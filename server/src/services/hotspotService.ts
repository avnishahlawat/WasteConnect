...
import { ServiceArea, type IServiceArea } from '../models/ServiceArea';
import { HotspotLevel, IssueStatus, Severity } from '../types/enums';
import mongoose from 'mongoose';

export interface HotspotCalculationDetails {
...
export const hotspotService = {
  getHotspotLevel(score: number): HotspotLevel {
    if (score <= 20) return HotspotLevel.LOW;
    if (score <= 40) return HotspotLevel.MODERATE;
    if (score <= 60) return HotspotLevel.ELEVATED;
    if (score <= 80) return HotspotLevel.HIGH;
    return HotspotLevel.CRITICAL;
  },

  async calculateAreaScore(serviceAreaId: mongoose.Types.ObjectId | string): Promise<HotspotCalculationDetails> {
  ...
  async recalculateAll(): Promise<HotspotCalculationDetails[]> {
    const areas = await ServiceArea.find({ isActive: true });
    const results: HotspotCalculationDetails[] = [];
