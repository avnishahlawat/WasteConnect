...
export const pickupStateService = {
  isValidTransition(currentStatus: PickupStatus, newStatus: PickupStatus): boolean {
  ...
  async transition(
    ...
    if (!this.isValidTransition(pickup.status, newStatus)) {
      throw new AppError(
        `Invalid status transition from ${pickup.status} to ${newStatus}`,
        400,
    ...
    if (newStatus === PickupStatus.CANCELLED) {
      ...
      if (note) pickup.cancellationReason = note;
    }
