...
import { pickupStateService } from './pickupStateService.js';
import { AppError } from '../utils/AppError.js';
import { PickupStatus, UserRole, NotificationType } from '../types/enums.js';
export const pickupService = {
    async createPickup(citizenId, input) {
        ...
        await CitizenProfile.findOneAndUpdate({ user: citizenId }, { $inc: { totalPickupsRequested: 1 } });
    ...
    async getCitizenPickups(citizenId, filters = {}) {
    ...
    async getPickupById(pickupId, userId, userRole) {
    ...
    async cancelPickup(pickupId, citizenId, reason) {
        ...
        if ([PickupStatus.COMPLETED, PickupStatus.CANCELLED].includes(pickup.status)) {
            throw new AppError(`Cannot cancel a pickup with status ${pickup.status}`, 400, 'CANNOT_CANCEL');
        }
        await pickupStateService.transition(pickup._id, PickupStatus.CANCELLED, citizenId, UserRole.CITIZEN, reason || 'Cancelled by citizen');
    ...
    async getAvailablePickupsForCollector(collectorId, filters = {}) {
        ...
        } else if (filters.onlyMyAreas && allowedAreas.length > 0) {
            query.serviceArea = { $in: allowedAreas };
        }

        // Filter by city / district if specified
        if (filters.city && filters.city !== 'ALL') {
            const cityAreas = await ServiceArea.find({ city: filters.city }).select('_id');
            const cityAreaIds = cityAreas.map((a) => a._id);
            if (!query.serviceArea) {
                query.serviceArea = { $in: cityAreaIds };
            }
        }
    ...
    async getCollectorPickups(collectorId, filters = {}) {
        const page = filters.page || 1;
        const limit = filters.limit || 20;
        const skip = (page - 1) * limit;
    ...
    async acceptPickup(pickupId, collectorId) {
        const pickup = await PickupRequest.findById(pickupId);
        if (!pickup)
            throw new AppError('Pickup not found', 404, 'PICKUP_NOT_FOUND');
        if (![PickupStatus.AVAILABLE, PickupStatus.PENDING].includes(pickup.status)) {
            throw new AppError('Pickup is no longer available for acceptance', 400, 'PICKUP_UNAVAILABLE');
        }
        await pickupStateService.transition(pickup._id, PickupStatus.ACCEPTED, collectorId, UserRole.COLLECTOR, 'Accepted by collector', { assignedCollector: collectorId });
        // Save assignment record
        await PickupAssignment.findOneAndUpdate({ pickupRequest: pickup._id }, { collector: collectorId, assignedBy: collectorId, assignedAt: new Date() }, { upsert: true });
        // Notify citizen
        await Notification.create({
    ...
    async startPickup(pickupId, collectorId) {
        const pickup = await PickupRequest.findOne({ _id: pickupId, assignedCollector: collectorId });
        if (!pickup)
            throw new AppError('Pickup not found or not assigned to you', 404, 'PICKUP_NOT_FOUND');
        await pickupStateService.transition(pickup._id, PickupStatus.IN_PROGRESS, collectorId, UserRole.COLLECTOR, 'Collector started collection route');
        await Notification.create({
            recipient: pickup.citizen,
    ...
    async completePickup(pickupId, collectorId, input) {
        const pickup = await PickupRequest.findOne({ _id: pickupId, assignedCollector: collectorId });
        if (!pickup)
            throw new AppError('Pickup not found or not assigned to you', 404, 'PICKUP_NOT_FOUND');
        ...
        await pickupStateService.transition(pickup._id, PickupStatus.COMPLETED, collectorId, UserRole.COLLECTOR, input.collectorNotes || 'Waste collection successfully completed', {
        ...
        await CollectorProfile.findOneAndUpdate({ user: collectorId }, {
            $inc: {
                totalCollections: 1,
        ...
        await CitizenProfile.findOneAndUpdate({ user: pickup.citizen }, { $inc: { totalPickupsCompleted: 1 } });
    ...
    async submitFeedback(pickupId, citizenId, input) {
        const pickup = await PickupRequest.findOne({ _id: pickupId, citizen: citizenId });
        if (!pickup)
            throw new AppError('Pickup not found', 404, 'PICKUP_NOT_FOUND');
        ...
        const avgRating = collectorFeedbacks.reduce((sum, f) => sum + f.rating, 0) / collectorFeedbacks.length;
        await CollectorProfile.findOneAndUpdate({ user: pickup.assignedCollector }, {
